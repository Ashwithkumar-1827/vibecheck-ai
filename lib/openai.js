...
CRITICAL BUG HUNTING CHECKLIST — CHECK FOR THESE SPECIFIC PROBLEMS:
1. React/JSX Tag Mismatches: Check all .js and .jsx React entry files (like index.js or App.js) for mismatched HTML/JSX tags. For example, opening tag <React.StrictMod> matched with closing tag </React.StrictMode>. This is a critical build/syntax failure!
2. Javascript RegExp/Syntax Errors: Check for malformed regex literals or unescaped characters, such as using "split(\\s+/)" or "split(\\s+/)" with a backslash outside of a regular expression literal (it should be split(/\\s+/) or similar).
3. Test Assertion Discrepancies: Compare the test files (like App.test.js or any test.js) with the components they render (like App.js). If a test file attempts to assert that a specific text, element, or link exists (such as screen.getByText(/learn react/i)), but that text/element is NOT present in the actual component files, this is an assertion logic error and MUST be reported as a "test" stage failure.
4. Logic Bugs & Broken Imports: Verify that imports resolve and no invalid objects/modules are required or referenced.
...
3. Syntax errors (missing brackets, broken expressions, invalid tokens, tag mismatches) should be reported as "build" stage failures.
