...
const db = {
  ...
  // Clear all builds and related data
  clearBuilds() {
    const data = readData();
    data.builds = [];
    data.patches = [];
  ...
  // Update proposed patch code contents
  updatePatchCode(patchId, originalCode, patchedCode, explanation = null) {
    const data = readData();
    const patch = data.patches.find(p => p.id === patchId);
    if (patch) {
      patch.original_code = originalCode;
      patch.patched_code = patchedCode;
      if (explanation !== null) {
