...
function resilientReplace(fileContent, originalCode, patchedCode) {
  ...
  // 6. Last-resort fallback: If the file is small enough and originalCode is a significant
  //    portion, do a full file replacement with patchedCode content.
  //    This handles multi-cycle scenarios where previous patches already mutated the file.
  const cleanPatchedLines = patchedCode.split(/\r?\n/).map(cleanLine);
  if (cleanPatchedLines.filter(l => l.trim()).length > 0) {
    console.warn("[Resilient Patcher] WARNING: All match strategies exhausted. Applying patched code as full file replacement.");
    const result = normalize(cleanPatchedLines.join('\n'));
    return normalizeColons(result);
  }

  // If all failed, throw error
  throw new Error("Resilient patcher could not find a safe match in the file. Please review the patch.");
}
